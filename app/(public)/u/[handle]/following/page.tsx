...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return { title: `@${handle} is following | Airworthiness`, robots: { index: false, follow: false } }
}
...
export default async function FollowingPage({ params }: PageProps) {
  ...
  const { data: following } = await supabase.rpc('get_following', { p_handle: handle, p_limit: 200 })
  const list: Entry[] = (following as Entry[]) ?? []
    ...
    <div className="bg-background">
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        <h1 className="text-2xl font-semibold text-foreground mt-3 mb-6">
          Following <span className="text-muted-foreground font-normal">· {list.length}</span>
        </h1>
        <FollowList
          entries={list}
