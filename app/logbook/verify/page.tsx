...
export default async function VerifyPage() {
  ...
  // Get the verifier's employment periods
  const { data: employmentPeriods } = await supabase
    .from('employment_periods')
    .select('employer, start_date, end_date')
    .eq('user_id', user.id)

  const verifierEmployers = employmentPeriods ?? []

  // Fetch all pending_verification entries (not owned by this user)
  ...
  // Filter: only entries where the verifier has authority (AML category match + employer overlap)
  const filteredEntries = (pendingEntries ?? []).filter(entry => {
    // Check AML category authority
    if (!canVerify(verifierCategories, entry.aircraft_category as AircraftCategory)) {
      return false
    }

    // Check employer overlap: verifier must have worked at same employer during the task date
    const taskDate = new Date(entry.task_date)
    return verifierEmployers.some(period => {
